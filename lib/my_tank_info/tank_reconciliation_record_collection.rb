...
module MyTankInfo
  class TankReconciliationRecordCollection
    ...
    def initialize(data:, reconciliation_period:)
      # auto_fill_needed means the record is not ready to be displayed to the user yet
      @data = data.select { _1.auto_fill_needed == false }
      @size = @data.size
      @reconciliation_period = reconciliation_period
    ...
    def tanks
      @data.map { |record|
        records =
          @data
            .select { _1.tank_number == record.tank_number }
            .sort_by(&:started_at)
